...
│   │   ├── 202522.yml
│   │   └── ...
│   └── unreliable-entity-list/             # 不可靠实体清单 (MOFCOM) - 14 files
│       ├── 202301.yml
│       ├── 202501.yml
│       └── ...
...
│                     │     │                     │     │                     │
│ • Announcement      │     │ • CnAnnouncement    │     │ • Entity nodes      │
│ • Sanction details  │     │ • CnEntity          │     │ • Entry nodes       │
│ • Legal instruments │     │ • CnMeasure         │     │ • Regime nodes      │
...
VERBOSE=true bundle exec rake generate
....
...
│   │   └── cn/
│   │       ├── anti-sanction-list/
│   │       │   └── {local_id}.jsonld     # Anti-sanction entries
│   │       ├── import-export-control-list/
│   │       │   └── {local_id}.jsonld     # Export control entries
│   │       └── unreliable-entity-list/
│   │           └── {local_id}.jsonld     # Unreliable entity entries
│   ├── regime/
│   │   ├── cn_anti_sanctions.jsonld
│   │   ├── cn_export_control.jsonld
│   │   └── cn_unreliable_entity.jsonld
│   └── authority/
│       └── cn.jsonld
├── by-list/                               # Entries grouped by list
├── by-authority/                          # Entries grouped by authority
├── by-type/                               # Entities grouped by type
...
The architecture follows a normalized pattern where entities and entries are separate but linked:

....
┌──────────────────────────────────────────────────────────────────────────┐
│                           ENTITY (List-Agnostic)                          │
│                                                                           │
│  id: https://www.ammitto.org/entity/cn/202535-csbc-corporation-taiwan    │
│  entity_type: organization                                                │
│  names:                                                                   │
│    - full_name: CSBC Corporation, Taiwan (Latn, primary)                 │
│    - full_name: 国际造船股份有限公司 (Hani)                              │
│  sanction_entry_ids:                                                      │
│    - https://www.ammitto.org/entry/cn/import-export-control-list/...     │
│                                                                           │
└───────────────────────────────────┬──────────────────────────────────────┘
                                    │
                                    │ sanction_entry_ids
...
│                        ENTRY (List-Specific)                              │
│                                                                           │
│  id: .../entry/cn/import-export-control-list/202535-csbc-corporation...  │
│  entity_id: .../entity/cn/202535-csbc-corporation-taiwan                 │
│  authority: { "@id": ".../authority/cn" }                                │
│  regime: { "@id": ".../regime/cn_export_control" }                       │
│  effects:                                                                 │
│    - effect_type: prohibit_export_dual_use_items                         │
│      description: 禁止向上述台湾地区实体出口两用物项...                   │
│    - effect_type: export_license_requirement                             │
│      description: 特殊情况下确需出口的...                                │
│  period:                                                                  │
│    effective_date: '2025-07-09'                                             │
│  status: active                                                           │
│  announcement:                                                            │
│    title: 公布将8家台湾地区实体列入出口管制管控名单                       │
│    url: https://www.mofcom.gov.cn/...                                    │
│    document_id: 2025年第35号                                              │
│    content: 根据《中华人民共和国出口管制法》...                          │
│                                                                           │
└──────────────────────────────────────────────────────────────────────────┘
....
...
All resources use normalized IRIs following these patterns:

[cols="1,3"]
|===
| Resource Type | IRI Pattern
...
| Regime
| `https://www.ammitto.org/regime/{regime_code}`
...
The transformer maps Chinese list names to URL-safe slugs:

[cols="2,2,2"]
|===
| Chinese Name | Internal Code | URL Slug
...
# Load entity
entity = JSON.parse(File.read('api/node/entity/cn/202535-csbc-corporation-taiwan.jsonld'))

# Get entity identity
puts entity['entity_type']  # => "organization"
puts entity['names'].first['full_name']  # => "CSBC Corporation, Taiwan"

# Get associated entries
entry_id = entity['sanction_entry_ids'].first
entry = JSON.parse(File.read("api/node/entry/cn/import-export-control-list/#{File.basename(entry_id)}.jsonld"))

# Get sanction effects
entry['effects'].each do |effect|
  puts "#{effect['effect_type']}: #{effect['description']}"
end
----

==== Querying by List Type

[source,ruby]
----
# Get all import-export control list entries
entries = Dir.glob('api/node/entry/cn/import-export-control-list/*.jsonld')

entries.each do |path|
  entry = JSON.parse(File.read(path))
  puts "#{entry['reference_number']}: #{entry['status']}"
end
----

=== Regime Codes

Each sanctions regime has a unique code for consistent referencing:
